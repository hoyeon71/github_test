...
public class Seed {
	...
	//private static String strMyKey = "ghayoun_01020304";
	//private static byte pbUserKey[]  = strMyKey.getBytes();
	...
	private static String encKey = "2013_ftvs_nsm214";
	...
	public static String setEncrypt(String str){
		...
		int strLength = (((bTemp.length-1)/16)+1);
		
		byte[] bPw = new byte[pw.length()];
		byte[] bStr	=	new byte[strLength*16];
		String encStr = "";
		bPw = pw.getBytes();
		int strSize = 0;
		int byteLength = 0;
		...
		{
			if ((i+1)*16< strLength*16){strSize =  (i+1)*16;}
			else {strSize =  bTemp.length;}
			byteLength = strSize-i*16;
			byte[] tempStr = new byte[byteLength];
			System.arraycopy(bTemp, i*16 ,tempStr, 0, byteLength);

			String returnStr = new String(tempStr);			
			
			pbData = padding(tempStr); // 16 자가 되지 않으면 (byte)0x00 으로 채워준다.

			returnStr = new String(pbData);			
			SeedRoundKey(pdwRoundKey, bPw);
			SeedEncrypt(pbData, pdwRoundKey, pbCipher);
			encStr += toString(pbCipher);
			System.arraycopy(pbCipher, 0 ,bStr, i*16, 16);//(원본,읽기시작,복사본,쓰기시작,length)
		}
		return toHexCode(bStr);
	}
	...
	public static String getDecrypt(String str){
		...
		str 	= CommonUtil.isNull(str,"");
		if(str.equals("")||str.length()%32>0){return str;}
		//System.out.println("암호원문 = "+str);
		String strTemp1 = "";
		byte[] strTemp2 = new byte[str.length()/2] ;
		String strTemp3 = "";
		int i = 0;
		int strInt = 0;
		String pw =encKey; 
		byte[] bPw = new byte[pw.length()];
		bPw = pw.getBytes();
		byte[] bResult = new byte[str.length()/2];

		
		for ( i=0 ; i<str.length()/2; i++)
		{
			strTemp3 = str.substring(i*2,i*2+2);
			strInt=Integer.parseInt(strTemp3,16);
			strTemp2[i] = (byte)strInt;
			strTemp1 += strInt;
		}
		...
		pbCipher = strTemp2;	// String을 byte[]로 변환
		String returnStr = new String(strTemp2);
		for (i=0; i< strTemp2.length/16; i++)
		{
			returnStr = new String(strTemp2);
			System.arraycopy(strTemp2, i*16 ,pbCipher, 0, 16);//(원본,읽기시작,복사본,쓰기시작,length)
			SeedDecrypt(pbCipher, pdwRoundKey, pbPlain);
		
			System.arraycopy(pbPlain, 0 ,bResult, i*16, pbPlain.length);//(원본,읽기시작,복사본,쓰기시작,length)
			returnStr = new String(bResult);
		}
		//System.out.println("복호중.. = "+returnStr);
		
		// 결과를 String 형으로 바꿈
		bResult = removePadding(bResult);
		returnStr = new String(bResult);
		//System.out.println("복호화결과 = ["+returnStr+"]");
		return returnStr;
	}
	...
	public static byte[] padding (byte[] str){
	...
	// byte값을 hex값으로 변환한다.
	public static String toHexCode(byte[] str){
		String return_text = "";
		String temp1 = "";
		int temp_int1 = 0;
		for(int i=0; i<str.length; i++){
			temp_int1 = (int)str[i];
			if (temp_int1 < 0){temp_int1 = temp_int1 + 256; }
			temp1 = Integer.toHexString(temp_int1) ;
			if (temp1.length()<2){temp1 = "0"+temp1;}
			return_text += temp1;
		}
		return return_text;
	}
		
	//복호시 padding을 제거한다. 0X80을 찾아서 그 위치 이후를 제거 
	public static byte[] removePadding (byte[] source){
		byte PADDING_VALUE = (byte)0x80;

		int paddingLocation = 0;
		for (int i=0 ; i<source.length;i++)
		{
			if(source[i] != PADDING_VALUE)
			{
			}
			else
			{
		...
		if(paddingLocation==0){paddingLocation=source.length;}
		byte[] paddingResult = new byte[paddingLocation];
		System.arraycopy(source, 0 ,paddingResult, 0, paddingLocation);//(원본,읽기시작,복사본,쓰기시작,length)				

		
		String returnStr = new String(paddingResult);
