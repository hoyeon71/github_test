...
;(function ( $, window, document, undefined ) {
	...
	ComboTree.prototype.selectMultipleItem=function(ctItem){
		...
		if($(ctItem).attr("data-id") == 'ALL') {
			var parentUl = $(ctItem).parent().parent();
			var span = parentUl.find("span");
			var input = parentUl.find("input");
			if( $(ctItem).find("input").is(':checked') ) {
				this._selectedItems = [];
				input.prop('checked', false);
			} else {
				
				// 멀티 콤보 전체 선택 시 기존 체크 되어 있는 조건에 전체를 ADD 하는 버그 개선 (2023.05.28 강명준)
				// 전체 체크 시 일단 초기화 후 전체 ADD
				this._selectedItems = [];
				
				for(var i=0; i < span.length; i++) {
					var __selectedItem = {
						id: $(span[i]).attr("data-id"),
            			title: $(span[i]).text()
					}
					this._selectedItems.push(__selectedItem);
				}
    ...
    ComboTree.prototype.refreshInputVal = function () {
        ...
        if (this.options.isMultiple) {
            for (var i=0; i<this._selectedItems.length; i++){
            	if( this._selectedItems[i].id == 'ALL' ) {
            		continue;
            	}
        ...
        $('#' + this.elemInputId + '_val').val(tmpId);
    }
