...
<%
	...
	String s_odate				= CommonUtil.getCurDateTo(CommonUtil.getCurDate(""), -1);
	String e_odate				= CommonUtil.getCurDateTo(CommonUtil.getCurDate(""), 0);

%>

<form id="f_s" name="f_s" method="post" onsubmit="return false;">
	
	<input type="hidden" id="itemGb2" 					name="itemGb2" 				value="jobLogList" />
	...
	<input type="hidden" name="active_gb" 				id="active_gb" 				value="0"/>
		...
		<td style='vertical-align:top;'>
			...
			<table style='width:100%;'>
				<tr>
					...
					<td width="25%" style="text-align:left width:300px;">
						<div class='cellContent_kang' style='width:300px;'>
						<select id="status" name="status" style="width:120px; height:21px;">
							<option value='' >--선택--</option>
							<%
								String aTmp[] = CommonUtil.getMessage("JOB_STATUS").split(",");
								for(int i=0; i<aTmp.length; i++){
									if(aTmp[i].equals("Ended OK") || aTmp[i].equals("Ended Not OK")){
							%>
									<option value="<%=aTmp[i]%>" ><%=CommonUtil.getMessage("JOB_STATUS."+aTmp[i].replaceAll(" ","_"))%></option>
							<%
									}
								}
							%>
					...
					<td width="20%" colspan=2>
						...
						<input type="text" name="s_odate" id="s_odate" value="${ODATE}" class="input datepick" style="width:75px; height:21px;" maxlength="8" /> ~
						<input type="text" name="e_odate" id="e_odate" value="${ODATE}" class="input datepick" style="width:75px; height:21px;" maxlength="8" />
						</div>
					</td>
				</tr>
				<tr>			
...
<!-- 							제외:<input type='checkbox' id='chk_app' name='chk_app' value = "N" />&nbsp;&nbsp; -->
				...
				<tr>
					...
					<td style="text-align:left;min-width:150px;" >
						<div class='cellContent_kang'>
						<input type="text" name="s_date1" id="s_date1" value="" class="input datepick" style="width:60px; height:21px;" maxlength="8" onClick="fn_clear(this.id);" placeholder = "시작일자"/>
						...
						~
						<input type="text" name="e_date1" id="e_date1" value="" class="input datepick" style="width:60px; height:21px;" maxlength="8" onClick="fn_clear(this.id);" placeholder = "시작일자"/>
...
// 			}
		...
		if(columnDef.id == 'SYS_OUT'){
			ret = "<a href=\"JavaScript:popupSysoutTelnet('"+order_id+"','"+run_cnt+"','"+end_date+"','"+mem_name+"','"+node_id+"','"+job_name+"','jobLog', '"+odate+"','"+run_cnt+"','"+sysout_yn+"');\" /><font color='red'>"+value+"</font></a>";
		}

		if(columnDef.id == 'SYS_OUT_DOWN'){
			ret = "<a href=\"JavaScript:popupSysoutTelnet_down('<%=sContextPath %>','"+order_id+"','"+run_cnt+"','"+end_date+"','"+mem_name+"','"+node_id+"','"+job_name+"','active');\" /><font>"+value+"</font></a>";
		}
		...
		if(columnDef.id == 'GRAPH'){
			ret = "<a href=\"JavaScript:popupJobGraph_d3('"+odate+"','"+order_id+"','"+job_name+"','"+ori_state_result+"');\" /><font color='red'>"+value+"</font></a>";
		}
	...
	var gridObj = {
		...
		,colModel:[
			...
			,{formatter:gridCellCustomFormatter,field:'SYS_OUT_DOWN',id:'SYS_OUT_DOWN',name:'다운',minWidth:70,headerCssClass:'cellCenter',cssClass:'cellCenter'}
	...
	$(document).ready(function(){
		...
		grid.onScroll.subscribe(function(e, args){
			var elem = $("#g_ez007").children(".slick-viewport"); 
...
// 				alert(elem[0].scrollHeight - elem.scrollTop() - elem.css("height").replace("px", "") + 17);
		...
		$.ajax({
			...
			success: function(data){
				...
				$.each(data, function(index, item){
					...
					var node_group 		= data[index].NODE_GROUP;
	...
	function popTimeInfoForm(job_name, order_id) {
		...
		dlPop01('dl_tmp1', "실행횟수 별 조회", 800, 500, false);
	...
	function timeInfoList(){
		...
		var url = '<%=sContextPath %>/common.ez?c=cData&itemGb=timeInfoList&itemGubun=2';
	...
	function goExcel(){
// 	  	var form 	= document.f_s;
// 	  	var frNm 	= "f_s";
<%-- 	  	var action 	= "<%=sContextPath %>/mEm.ez?c=ez007_excel"; --%>
// 	  	var target 	= "if1";
	  	
// 	  	fileDownProg(form ,frNm, action, target);
	  	
		var frm = document.f_s;
		frm.action = "<%=sContextPath %>/mEm.ez?c=ez007_excel";
		frm.target = "if1";
		frm.submit();
	}
	...
	function popupJobGraph_d3(odate, order_id, job_name, status) {
	...
	function popupSysoutTelnet(order_id, rerun_count, end_date, mem_name, node_id, job_name, flag, odate, run_count, sysout_yn) {
	...
	function popupAjobInfo(order_id, job_name, active_gb) {
		...
		openPopupCenter("about:blank","popupAjobInfo1", 800, 600);
	...
	function popupTimeInfo(job_name,memname){
		...
		openPopupCenter("about:blank","popupTimeInfo", 600, 500);
