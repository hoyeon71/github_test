...
<%
	...
	List<com.ghayoun.ezjobs.t.domain.ActiveJobBean> activeJobList = (List)request.getAttribute("activeJobList");
...
<%
	try {
		...
		timeFromMaxCellWidth = Math.max(timeFromMaxCellWidth, cellWidth);
		...
		jobSchedMaxCellWidth = Math.max(jobSchedMaxCellWidth, cellWidth);
		...
		setCellValue(r, ++n, "ORDER_ID", style);
		cellWidth = "ORDER_ID".getBytes("UTF-8").length * 256;
		...
		for( int i=0; null!=activeJobList && i<activeJobList.size(); i++ ){
			
			com.ghayoun.ezjobs.t.domain.ActiveJobBean bean = (com.ghayoun.ezjobs.t.domain.ActiveJobBean)activeJobList.get(i);
			...
			String strTimeFrom		= CommonUtil.isNull(bean.getTime_from());
			...
			String strSetValue		= CommonUtil.isNull(bean.getSet_value());
...
// 			String strTeam_nm		= CommonUtil.isNull(bean.getTeam_nm());
			
			String strStateResult2 	= "";
			String strOrder_id		= CommonUtil.isNull(bean.getOrder_id());
			...
			setCellValue(r, ++n, strTimeFrom, style);
			cellWidth = strTimeFrom.getBytes("UTF-8").length * 256;
			timeFromMaxCellWidth = Math.max(timeFromMaxCellWidth, cellWidth);
			...
			setCellValue(r, ++n, strOrder_id, style);
			cellWidth = strOrder_id.getBytes("UTF-8").length * 256;
			orderIdMaxCellWidth = Math.max(orderIdMaxCellWidth, cellWidth);
		...
		CommonUtil.setAutoSizeColumn(sheet, ++n, timeFromMaxCellWidth);
		...
		CommonUtil.setAutoSizeColumn(sheet, ++n, jobSchedMaxCellWidth);
