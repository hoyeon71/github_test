...
BEGIN
    ...
    if p_flag ='temp_ins' then
        begin
            /*
			SELECT 'EZJ' || TO_CHAR(current_timestamp,'yyyymmdd') || LPAD((COALESCE(REPLACE(MAX(doc_cd),'EZJ'|| TO_CHAR(current_timestamp,'yyyymmdd'),'')::integer,0)+1)::text,5,'0')
			INTO v_max_doc_cd
			from ezjobs.ez_doc_master
			where doc_cd like 'EZJ'|| to_char(current_timestamp,'yyyymmdd') || '%' ;
			*/
    ...
    if p_flag ='draft_u' or p_flag ='post_draft_u' or p_flag ='draft_u_admin' then
        begin
            ...
            if p_flag ='draft_u' or p_flag ='post_draft_u' or p_flag ='draft_u_admin' then
                begin
                    ...
                    set
                      ...
                      ,cur_approval_seq = 1
                      ,post_approval_yn = (case when p_flag = 'draft_u_admin' then 'A' else p_post_approval_yn end)
                    ...
                    set title = (CASE WHEN p_flag = 'draft_u_admin' then '[관리자 즉시결재]' ELSE p_title END)
                    where doc_cd = p_doc_cd
                    ;
                    ...
                    if p_flag ='draft_u' or p_flag ='post_draft_u' then
                        begin
                            ...
                            WHERE use_yn = 'Y'
                              AND post_approval_yn = case when p_flag = 'post_draft_i' or p_flag = 'post_draft' then 'Y' else 'N' end
                              AND doc_gubun = '01';
...
BEGIN
    ...
    if p_flag ='draft' or p_flag ='post_draft_u' or p_flag ='draft_admin' then
        begin
            ...
            if p_flag ='draft' or p_flag ='post_draft_u' or p_flag ='draft_admin' then
                begin
                    ...
                    if p_flag ='draft' or p_flag ='post_draft_u' then
                        begin
                            ...
                            FROM ezjobs.EZ_ADMIN_APPROVAL_GROUP
                            WHERE use_yn = 'Y'
                              AND post_approval_yn = (CASE WHEN p_flag = 'post_draft_u' THEN 'Y' ELSE 'N' END)
                              AND doc_gubun = '01';
