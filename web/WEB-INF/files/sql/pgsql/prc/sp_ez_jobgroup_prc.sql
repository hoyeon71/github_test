-- DROP FUNCTION ezjobs4.sp_ez_jobgroup_prc(out varchar, out varchar, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar, in varchar);

CREATE OR REPLACE FUNCTION ezjobs4.sp_ez_jobgroup_prc(OUT r_code character varying, OUT r_msg character varying, p_flag character varying, p_table_id character varying, p_job_id character varying, p_data_center character varying, p_job_name character varying, p_user_cd character varying, p_jobgroup_id character varying, p_jobgroup_name character varying, p_content character varying, p_table_name character varying, p_application character varying, p_group_name character varying)
 RETURNS record
 LANGUAGE plpgsql
AS $function$

DECLARE

    v_chk_cnt numeric;
    v_table_id 	numeric;
    v_job_id 	numeric;

    rec_affected 	numeric;

BEGIN

    if p_flag ='detail_insert' then
        begin

            INSERT INTO ezjobs4.EZ_JOBGROUP_JOB (
                jobgroup_id,
                table_id,
                job_id,
                data_center,
                job_name,
                table_name,
                application,
                group_name,
                ins_date
            )
            VALUES (
                       p_jobgroup_id,
                       p_table_id::integer,
                       p_job_id::integer,
                       p_data_center,
                       p_job_name,
                       p_table_name,
                       p_application,
                       p_group_name,
                       current_timestamp
                   );

            GET DIAGNOSTICS rec_affected := ROW_COUNT;
            if rec_affected < 1 then
                begin
                    r_code := '-1';
                    r_msg := 'ERROR.01';
                    RAISE EXCEPTION 'rec_affected 0';
                end;
            end if;

        end;
    end if;

    if p_flag ='detail_delete' then
        begin

            DELETE FROM ezjobs4.EZ_JOBGROUP_JOB
            WHERE jobgroup_id   	= p_jobgroup_id
              AND job_name       	= p_job_name
              AND data_center   	= p_data_center
              and table_name 		= p_table_name
              and application 		= p_application
              and group_name		= p_group_name;

            GET DIAGNOSTICS rec_affected := ROW_COUNT;
            if rec_affected < 1 then
                begin
                    r_code := '-1';
                    r_msg := 'ERROR.01';
                    RAISE EXCEPTION 'rec_affected 0';
                end;
            end if;

        end;
    end if;

    if p_flag ='group_insert' then
        begin

            SELECT COUNT(*)
            INTO v_chk_cnt
            FROM ezjobs4.EZ_JOBGROUP
            WHERE jobgroup_name = p_jobgroup_name;


            if v_chk_cnt > 0 then
                begin
                    r_code := '-1';
                    r_msg := 'ERROR.24';
                    RAISE EXCEPTION 'rec_affected 0';
                end;
            end if;

            INSERT INTO ezjobs4.EZ_JOBGROUP (
                jobgroup_id,
                jobgroup_name,
                ins_user_cd,
                ins_date,
                content,
                udt_user_cd,
                udt_date
            )
            VALUES (
                       (SELECT 'G' || ezjobs4.NVL(MAX(REPLACE(jobgroup_id, 'G', '')::integer+1), 1) FROM ezjobs4.EZ_JOBGROUP),
                       p_jobgroup_name,
                       p_user_cd::integer,
                       current_timestamp,
                       p_content,
                       p_user_cd::integer,
                       current_timestamp
                   );

            GET DIAGNOSTICS rec_affected := ROW_COUNT;
            if rec_affected < 1 then
                begin
                    r_code := '-1';
                    r_msg := 'ERROR.01';
                    RAISE EXCEPTION 'rec_affected 0';
                end;
            end if;

        end;
    end if;

    if p_flag ='group_update' then
        begin

            UPDATE ezjobs4.EZ_JOBGROUP SET
                                           jobgroup_name  = p_jobgroup_name,
                                           udt_user_cd    = p_user_cd::integer,
				   udt_date       = current_timestamp,  
				   content        = p_content
            WHERE jobgroup_id   = p_jobgroup_id;

            GET DIAGNOSTICS rec_affected := ROW_COUNT;
            if rec_affected < 1 then
                begin
                    r_code := '-1';
                    r_msg := 'ERROR.01';
                    RAISE EXCEPTION 'rec_affected 0';
                end;
            end if;

        end;
    end if;

    if p_flag ='group_delete' then
        begin

            DELETE FROM ezjobs4.EZ_JOBGROUP_JOB
            WHERE jobgroup_id   = p_jobgroup_id;

            DELETE FROM ezjobs4.EZ_JOBGROUP
            WHERE jobgroup_id   = p_jobgroup_id;

            GET DIAGNOSTICS rec_affected := ROW_COUNT;
            if rec_affected < 1 then
                begin
                    r_code := '-1';
                    r_msg := 'ERROR.01';
                    RAISE EXCEPTION 'rec_affected 0';
                end;
            end if;

        end;
    end if;

    if p_flag ='update_job_id' then
        begin

            SELECT MAX(tb1.table_id)
            INTO v_table_id
            FROM DEF_JOB tb1, DEF_TABLES tb2
            WHERE tb1.table_id = tb2.table_id
              AND tb2.data_center = p_data_center
              AND tb1.job_name = p_job_name;

            SELECT MAX(tb1.job_id)
            INTO v_job_id
            FROM DEF_JOB tb1, DEF_TABLES tb2
            WHERE tb1.table_id = tb2.table_id
              AND tb2.data_center = p_data_center
              AND tb1.job_name = p_job_name;

            UPDATE ezjobs4.EZ_JOBGROUP_JOB
            SET table_id = v_table_id, job_id = v_job_id
            WHERE data_center = p_data_center
              AND job_name = p_job_name;

        end;
    end if;

    if p_flag ='delete_job_id' then
        begin

            DELETE FROM ezjobs4.EZ_JOBGROUP_JOB
            WHERE data_center = p_data_center
              AND job_name = p_job_name;
            --		         WHERE table_id = (case when p_table_id is not null and length(p_table_id)>0 then cast(p_table_id as numeric) else 0 end)
--		           AND job_id =(case when p_job_id is not null and length(p_job_id)>0 then cast(p_job_id as numeric) else 0 end);

        end;
    end if;

    r_code := '1';
    r_msg := 'DEBUG.01';
    return;

    --EXCEPTION
--	WHEN OTHERS THEN
--		r_code := '-1';
--		if r_msg IS NULL then
--			r_msg := 'ERROR.01';
--		end if;
--		r_code := '-2';
--		r_msg := SQLERRM;
--	
--END;

EXCEPTION
    WHEN OTHERS THEN
        IF r_code = '-1' THEN
            IF r_msg IS NULL THEN
                r_msg := 'ERROR.01';
            END IF;
        ELSE
            r_code := '-2';
            r_msg := SQLERRM;
        END IF;
END;
$function$
;
